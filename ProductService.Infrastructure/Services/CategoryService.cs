    ...
    public class CategoryService : ICategoryService
    {
        ...
        public async Task<DataResponse<Categories>> AddAsync(Categories data)
        {
            try
            {
                ...
                if (saveChange == 0)
                {
                    return new DataResponse<Categories>(saveChange, "Thêm không thành công!");
                }
        ...
        public async Task<DataResponse<bool>> DeleteAsync(string categoryID)
        {
            try
            {
                var category = await _unitOfWork.Repository<Categories>().GetOneAsync(x => x.CategoryID == categoryID);
                if (category == null)
                {
