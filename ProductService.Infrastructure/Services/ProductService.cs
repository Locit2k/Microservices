...
namespace ProductService.Infrastructure.Services
{
    public class ProductService : IProductService
    {
        ...
        public async Task<DataResponse<ProductDTO>> UpdateAsync(ProductDTO data)
        {
            try
            {
                var product = await _unitOfWork.Repository<Products>().GetOneAsync(x => x.ProductID == data.ProductID);
                if (product == null)
                {
        ...
        public async Task<DataResponse<bool>> DeleteAsync(Guid productID)
        {
            try
            {
                var product = await _unitOfWork.Repository<Products>().GetOneAsync(x => x.ProductID == productID);
                if (product == null)
                {
